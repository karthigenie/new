...
const fetchPosts = async ({ pageParam = 1 }) => {
  const response = await fetch(
    `http://localhost:5000/video/get?page=${pageParam}&limit=5`
  );
...
export default function Home() {
 ...
 /*    axios.get(`http://localhost:5000/get?page=${id}`).then(res => setRes(res.data)) */
  ...
  if(links.length === 0){
    ...
    formData.append('format',"video")
       ...
       await axios.post('http://localhost:5000/video',formData).then(res => alert("Upload Successfull")).catch(error => alert(error))
  ...
  const submitAudio = async (event) => {
    ...
    if(audiolinks.length !==0){
      ...
      try {
        await axios({
         method: "post",
         url: "http://localhost:5000/audio/url",
         data: body,
       }).then(res => alert("Upload Successfull"));
     } catch(error) {
       alert(error)
    ...
    await axios.post('http://localhost:5000/audio',formData).then(res => alert("Upload Successfull")).catch(error => alert(error))
  }
  }
  const submitImage = async (event) => {
    ...
    await axios.post('http://localhost:5000/image',formData).then(res => alert("Upload Successfull")).catch(error => alert(error))
  }
  }
